...
containers.forEach(container =>{
	...
	const groupId = group.getAttribute('value');
  ...
  kids.forEach(kid => {
	  ...
	  kidinput.forEach(inp=> {
		  if(event.currentTarget['id']=="selectedContacts"){
			  const contactId = inp.getAttribute('value')
		  ...
		  }else if(event.currentTarget['id']=="availableContacts"){
			  const contactId = inp.getAttribute('value')
			  console.log('add group '+groupId+' to contact '+contactId);
